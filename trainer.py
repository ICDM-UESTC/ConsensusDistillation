...
class BaseTrainer(object):
    ...
    def calculate_shd_auc_fid_acc(self, dataset_name, method_name, ensemble_numbers=[0]):
        """
        fid- can be only calculated for single model, shd, auc, acc can be calculated after EE
        """
        assert self.cfg.multi_label is False  # only support binary classification
        ...
        for model_index in ensemble_numbers:
            ...
            # new_checkpoints_path = f'./outputs/archived_checkpoints/{self.method_name}_{self.cfg.dataset_name}_{model_index}.pth'
            ...
            for data_index, data in enumerate(self.dataloader['test_by_sample']):
                ...
                minus_att = self.concrete_sample(self.explainer(minus_emb, data.edge_index, data.batch),
                                                 training=False)
                ...
                ori_data[data_index][f'y_hat_{model_index}'] = logits.sigmoid()
                ori_data[data_index][f'y_hat_minus_{model_index}'] = logits_minus.sigmoid()
...
class ATTCDTrainer(BaseTrainer):
    ...
    def __loss__(self, att, logits, labels, att_cd, att_ee_mean, att_ee_std):
        ce_loss = self.criterion(logits, labels)
        distill_loss = torch.mean(torch.abs(att_cd - att_ee_mean))
    ...
    def _train_batch(self, data, is_test=True):
        ...
        mean_edge_att = all_ee_edge_atts.mean(dim=0)
        std_edge_att = all_ee_edge_atts.std(dim=0)

        max_std = torch.max(std_edge_att).item()
        if max_std > self.max_std:
            self.max_std = max_std

        loss, distill_loss = self.__loss__(att=att, logits=clf_logits, labels=data.y, att_cd=edge_att_cd,
                                           att_ee_mean=mean_edge_att,
                                           att_ee_std=std_edge_att)
    ...
    def test(self):
        ...
        # path = f'./outputs/archived_checkpoints/{self.method_name[:-3]}_{self.cfg.dataset_name}_{0}.pth'
        # state = torch.load(path, map_location=self.device)
        # self.model.load_state_dict(state['model_state_dict'])
        # self.explainer.load_state_dict(state['explainer_state_dict'])
    ...
    def train_ft(self, cur_index):
        ...
        for e in range(self.cfg.epochs):
            ...
            print(f"epoch: {e}, "
                  f"train loss: {train_loss:.4f}, train acc {train_metrics['acc']:.4f}, "
                  f"valid loss: {valid_loss:.4f}, valid acc {valid_metrics['acc']:.4f}")
            if (self.c == self.sparsity_mask_coef) and (
                    (valid_metrics['acc'] > self.best_valid_score) or
                    (valid_metrics['acc'] == self.best_valid_score and valid_loss < self.lowest_valid_loss)):
                self.save_model(self.checkpoints_path)
                self.best_valid_score = valid_metrics['acc']
    ...
    def train_ft(self, cur_index):
        ...
        for e in range(self.cfg.epochs):
            ...
            print(f"epoch: {e}, "
                  f"train loss: {train_loss:.4f}, train acc {train_metrics['acc']:.4f}, "
                  f"valid loss: {valid_loss:.4f}, valid acc {valid_metrics['acc']:.4f}")
            # if (self.r == self.final_r) and (e > 10) and ((valid_metrics['acc'] > self.best_valid_score) or (
            #         valid_metrics['acc'] == self.best_valid_score and valid_loss < self.lowest_valid_loss)):
            if (e > 10) and ((valid_metrics['acc'] > self.best_valid_score) or (
                    valid_metrics['acc'] == self.best_valid_score and valid_loss < self.lowest_valid_loss)):
                self.save_model(self.checkpoints_path)
                self.best_valid_score = valid_metrics['acc']
                self.lowest_valid_loss = valid_loss
...
class CALCDTrainer(BaseTrainer):
    ...
    def __loss__(self, c_logits, s_logits, csi_logits, labels, att_cd, att_ee_mean, att_ee_std):
        if self.num_class == 2:
            s_prob = torch.sigmoid(s_logits)
            s_loss = (s_prob * torch.log(s_prob / self.r + 1e-6)
                      + (1 - s_prob) * torch.log((1 - s_prob) / (1 - self.r + 1e-6) + 1e-6)).mean()
